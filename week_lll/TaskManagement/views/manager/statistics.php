...
<main id="main" class="main">
    ...
    <div class="container">
        ...
        // Tasks
        $taks_complated = getTaskCountByManagerAndStatus($user_id, 5);
        if (!$taks_complated) {
            returnResponse('hass error when get taks_complated');
        }
        $taks_NotAssigned = getTaskCountByManagerAndStatus($user_id, 0);
        if (!$taks_NotAssigned) {
            returnResponse('hass error when get taks_NotAssigned');
        }

        // Employees
        $employee_active = EmployeesByManager(1, $user_id);
        if (!$employee_active) {
            returnResponse('hass error when get employee_active');
        }
        $employee_not_active = EmployeesByManager(0, $user_id);
        if (!$employee_not_active) {
            returnResponse('hass error when get employee_not_active');
        }
        $employee_disabled = EmployeesByManager(2, $user_id);
